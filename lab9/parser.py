...
class MiniCParser(Parser):
    ...
    def array_decl(self, p):
        return ArrayDecl(p.type_spec, p.ID, int(p.NUMBER))
    ...
    def func_decl(self, p):
        params = p.param_list if hasattr(p, 'param_list') else []
        return FuncDecl(p.type_spec, p.ID, params, p.compound_stmt)
    ...
    def param(self, p):
        return ArrayDecl(p.type_spec, p.ID, 0)
