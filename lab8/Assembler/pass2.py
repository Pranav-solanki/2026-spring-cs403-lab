...
class ObjectCode:
    ...
    def generate(instruction: SICTuple, loc: int) -> Optional[str]:
        ...
        print(f"DEBUG: Generating object code for: {mnemonic}, {operand} at loc {loc:X}")  # Debug statement
        if mnemonic is '':
            # This can happen for lines that are just labels (e.g., "LOOP:") or empty lines. No code generated.
            return None
...
class Pass2Assembler:
    ...
    def run(self):
        ...
        for line in self.source_file:
            loc, tup = IntermediateLineParser.parse(line.strip('\n'))
            if not tup: continue
            if tup.mnemonic == 'START':
                continue

            if tup.mnemonic == 'END':
                self._flush_text_record()
                # Determine entry point
                entry_label = tup.operand
                entry_addr = self.start_addr
                if entry_label:
                    sym_addr = bindings.search_symbol(entry_label.encode('utf-8'))
                    if sym_addr is not None and sym_addr != -1:
                        entry_addr = sym_addr

                self._write_end(entry_addr)
                break

            # Handle Memory Reservation Gaps
            if tup.mnemonic in ['RESW', 'RESB']:
                self._flush_text_record()
                continue

            # Generate Object Code
            obj_code = ObjectCode.generate(tup, loc)
            # self._debug_code(tup, loc, obj_code)
            if obj_code:
                self._add_to_text_record(obj_code, loc)